...
export function primeFactorization(value) {
  ...
  while (divisor * divisor <= remaining) {
    if (remaining % divisor === 0) {
      const entry = factors.find((item) => item.prime === divisor);
      if (entry) {
        entry.exponent += 1;
      } else {
        factors.push({ prime: divisor, exponent: 1 });
      }
  ...
  if (remaining > 1) {
    const entry = factors.find((item) => item.prime === remaining);
    if (entry) {
      entry.exponent += 1;
    } else {
...
export function simplifyFraction(numerator, denominator) {
  ...
  const divisor = gcd(Math.round(normalizedNumerator), Math.round(normalizedDenominator));
  return {
    numerator: Math.round(normalizedNumerator / divisor),
    denominator: Math.round(normalizedDenominator / divisor),
  };
}

export function operateFractions({ aNumerator, aDenominator, bNumerator, bDenominator, operation }) {
  if (
    !isFiniteNumber(aNumerator) ||
    !isFiniteNumber(aDenominator) ||
    !isFiniteNumber(bNumerator) ||
    !isFiniteNumber(bDenominator) ||
    aDenominator === 0 ||
    bDenominator === 0
  ) {
    return { numerator: 0, denominator: 1 };
  ...
  switch (operation) {
    case 'add':
      numerator = aNumerator * bDenominator + bNumerator * aDenominator;
    ...
    case 'divide':
      ...
      denominator = aDenominator * bNumerator;
      break;
    default:
      numerator = 0;
      denominator = 1;
...
export function convertArea(value, from, to) {
  ...
  const squareMeters = value * areaToSquareMeter[from];
  return round(squareMeters / areaToSquareMeter[to], 6);
}
...
export function convertEnergy(value, from, to) {
  ...
  const joules = value * energyToJoule[from];
  return round(joules / energyToJoule[to], 6);
}
...
export function triangleAreaBaseHeight(base, height) {
  if (!isFiniteNumber(base) || !isFiniteNumber(height)) {
    return 0;
  }
  return round((base * height) / 2, 4);
}
...
export function triangleAreaHeron(a, b, c) {
  ...
  return round(Math.sqrt(areaSquared), 4);
}

export function triangleAreaTwoSidesAngle(a, b, angleDegrees) {
  if (!isFiniteNumber(a) || !isFiniteNumber(b) || !isFiniteNumber(angleDegrees)) {
    return 0;
  }
  const radians = (angleDegrees * Math.PI) / 180;
  return round((a * b * Math.sin(radians)) / 2, 4);
}
...
export const areaUnits = {
  ...
  squareCentimeter: 'Quadratzentimeter (cm²)',
  squareMillimeter: 'Quadratmillimeter (mm²)',
  hectare: 'Hektar (ha)',
  acre: 'Acre',
  squareFoot: 'Quadratfuß (ft²)',
  squareInch: 'Quadratzoll (in²)',
};
...
export const energyUnits = {
  ...
  wattHour: 'Wattstunde (Wh)',
  kilowattHour: 'Kilowattstunde (kWh)',
  electronVolt: 'Elektronenvolt (eV)',
