...
async function loadRates() {
  ...
  try {
    ...
    const payload = { timestamp: now, rates, date: dateCube };
    persistState(FX_STORAGE_KEY, payload);
    return payload;
  } catch (error) {
    if (cached && cached.timestamp && now - cached.timestamp < 48 * 60 * 60 * 1000) {
      return cached;
    }
    throw error;
  }
...
function renderCurrencyTool(container) {
  ...
  function compute() {
    ...
    rateInfo.textContent = `EZB-Referenzkurs vom ${rates.date ?? 'aktuellen Handelstag'}`;
